...
from users.models import Follow, Profile, Ban
import sys
...
class UserPostListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        visible_user = self.visible_user()  # пользователь
        logged_user = self.request.user  # текущий пользователь
        ...
        # Банищий
        banned = Ban.objects.filter(who_blocking=logged_user, blocked_users=visible_user).count() == 0
        # Тот кто забанин
        ban = Ban.objects.filter(who_blocking=visible_user, blocked_users=logged_user).count() == 0

        if banned == False or ban == False:
            follows = Follow.objects.filter(user=logged_user, follow_user=self.visible_user())
            if follows.count() > 0:
                follows.delete()
    ...
    def post(self, request, *args, **kwargs):
        if request.user.id is not None:
            ...
            banned = Ban.objects.filter(who_blocking=request.user,
            ...
            if banned == True:
                ...
                if 'follow' in request.POST:
                    ...
                    new_ban = Ban(who_blocking=request.user, blocked_users=self.visible_user())
